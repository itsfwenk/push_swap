...
/*   Updated: 2024/07/16 16:42:26 by fli              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	check_instructs(char *instruct)
{
	if (ft_strncmp(instruct, "sa\n", ft_strlen("sa\n")) == 0
		|| ft_strncmp(instruct, "pa\n", ft_strlen("pa\n")) == 0
		|| ft_strncmp(instruct, "ra\n", ft_strlen("ra\n")) == 0
		|| ft_strncmp(instruct, "rra\n", ft_strlen("rra\n")) == 0
		|| ft_strncmp(instruct, "sb\n", ft_strlen("sb\n")) == 0
		|| ft_strncmp(instruct, "pb\n", ft_strlen("pb\n")) == 0
		|| ft_strncmp(instruct, "rb\n", ft_strlen("rb\n")) == 0
		|| ft_strncmp(instruct, "rrb\n", ft_strlen("rrb\n")) == 0
		|| ft_strncmp(instruct, "ss\n", ft_strlen("ss\n")) == 0
		|| ft_strncmp(instruct, "rr\n", ft_strlen("rr\n")) == 0
		|| ft_strncmp(instruct, "rrr\n", ft_strlen("rrr\n")) == 0)
		return (TRUE);
	return (FALSE);
}

static int	get_instructs(t_pile **pile_a, t_pile **pile_b, int **num_tab)
{
	char	*instruct;

	instruct = get_next_line(0);
	if (instruct == NULL)
		return (free(instruct), get_next_line(-1), FALSE);
	while (instruct != NULL)
	{
		if (check_instructs(instruct) == FALSE)
		{
			write(2, "Error\n", 7);
...
int	main(int argc, char **argv)
{
	...
	if (create_pile_a(tab_len, &pile_a, num_tab) == FALSE)
		return (-1);
	pile_b = NULL;
	while (get_instructs(&pile_a, &pile_b, &num_tab) == TRUE)
		;
